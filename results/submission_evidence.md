...
- Multi-turn evaluation report: [`evaluation_report.md`](evaluation_report.md)
- Live demo verification report: [`demo_verification.md`](demo_verification.md)
...
- Multi-turn evaluation report: [`evaluation_report.md`](evaluation_report.md)
- Multi-turn evaluation CSV: [`multiturn_eval_results.csv`](multiturn_eval_results.csv)
- Multi-turn evaluation JSON summary: [`multiturn_eval_summary.json`](multiturn_eval_summary.json)
...
- deterministic multi-turn metrics are generated by `python scripts/evaluate_multiturn.py --output-dir results`
- the report includes condition accuracy, urgency accuracy, under-escalation rate, over-escalation rate, JSON-validity rate, mean reward, and mean steps
- trained-model improvement should be claimed only after saved model action traces are run through the same evaluator with `--predictions-jsonl`
...
- Verify the OpenEnv demo before presentations with:
...
- Reward hardening for common label aliases, invalid-label penalties, trajectory-target alignment, and explicit under-escalation reporting
