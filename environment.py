...
try:
    ...
    # Fallback if openenv-core not installed locally — still works on HF
    class OpenEnvBase:
        pass
...
LATENT_CONDITION_ADVICE = {
    "iron_deficiency":   "Include iron-rich foods like spinach and jaggery daily",
    "dehydration":       "Drink at least 10 glasses of water per day",
    "poor_sleep":        "Sleep at least 8 hours; rest with legs elevated",
    "nutritional_gap":   "Eat 3 proper meals and 2 light snacks daily",
}
...
# REWARD FUNCTION — tiered for GRPO training signal diversity
...
def _calculate_reward(predicted: str,
    ...
    # Tier 1 — condition field exists and is a valid label (+0.2)
    if predicted in CONDITIONS:
        reward += 0.2

    # Tier 2 — urgency field exists and is valid (+0.2)
    if predicted_urgency in URGENCY_LEVELS:
        reward += 0.2

    # Tier 3 — correct condition (+0.3 / -0.3)
    if predicted == true_condition:
        reward += 0.3
    else:
        reward -= 0.3

    # Tier 4 — correct urgency (+0.2), over-escalation (-0.1), under-escalation (-(severity * 0.05))
    if predicted_urgency == true_urgency:
        reward += 0.2
    elif urgency_idx(predicted_urgency) > urgency_idx(true_urgency):
        reward -= 0.1   # cautious over-escalation — small penalty
    else:
        reward -= severity * 0.05  # dangerous under-escalation

    # Bonus — more data available
    if days_of_data >= 3:
        reward += 0.1
    elif days_of_data == 2:
        reward += 0.05

    # Final range roughly: -0.65 to +1.0
    return round(reward, 4)


# MAIN ENVIRONMENT CLASS
# Inherits from OpenEnv base class — required by OpenEnv competition spec

class PrenatalEnvironment(OpenEnvBase):
    """
    OpenEnv-compatible RL environment for Niva — maternal health AI.

    Inherits from openenv.Environment per OpenEnv submission spec.
    ...
    step() / reset() / state() follow the standard Gym-style OpenEnv API.
    """
    ...
    def __init__(self):
        ...
        self.true_condition:   Optional[str]          = None
    ...
    def step(self, action: ActionModel) -> StepResult:
        ...
        action_type = "assess"   → returns observation, no reward yet
        action_type = "diagnose" → commits to condition + urgency, gets reward, episode ends
        """
        ...
        # ── DIAGNOSE: agent makes final call ──────────────
            ...
            # Safety override — DANGER flags must escalate to hospital
            ...
            latent_advice   = [
                LATENT_CONDITION_ADVICE[k]
                for k in policy_snapshot.latent_risks
                if k in LATENT_CONDITION_ADVICE
            ]
            ...
            raise ValueError(
                f"Unknown action_type: {action.action_type}. Use 'assess' or 'diagnose'."
            )
